...
const createcompanyInfo = async (req, res) => {
    ...
    try {
        ...
        const companyInfo = new Customer(req.body);
...
const getAll = async (req, res) => {
  try {
      const getcompanyInfo = await Customer.find();
...
const deleteById = async (req, res) => {
  ...
  try {
      const deletedInfo = await Customer.findByIdAndDelete({_id:id});
      if (!deletedInfo) {
...
const getById = async (req, res) => {
  const id = req.query.id;
  try {
      const info = await Customer.findById({_id:id});
      if (!info) {
          return res.status(404).json({ message: "Company info not found" });
      }
...
const updateById = async (req, res) => {
  const id = req.query.id;
  try {
      const updatedInfo = await Customer.findByIdAndUpdate({_id:id}, req.body, { new: true });
      if (!updatedInfo) {
          return res.status(404).json({ message: "Company info not found" });
      }
